...
def stochastic_approximation(theta_0, stepsize, n_smaples, decreasing_stepsize=False, computational_budget=5000):
    # setup parameters
    theta = theta_0
    S = 20
    h = 4
...
if __name__ == "__main__":
    ...
    for e, gain in enumerate(fixed_gains):
        axis[0].plot(stochastic_approximation(standard_theta, gain, standard_n_samples,
                                              computational_budget=computational_budget), label=gain, alpha=0.95)
        axis[0].set_title('fixed gain')
        axis[0].set_xlabel('iterations (n)')
        axis[0].set_ylabel('Theta')
        axis[0].legend()
    for e, gain in enumerate(decreasing_gains):
        axis[1].plot(stochastic_approximation(standard_theta, gain, standard_n_samples, decreasing_stepsize=True,
                                              computational_budget=computational_budget), label=gain, alpha=0.95)
